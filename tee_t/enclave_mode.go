...
func startEnclaveMode(config *TEETConfig) {
	...
	teet := NewTEETWithEnclaveManager(int(enclaveConfig.HTTPSPort), enclaveManager)
...
func setupEnclaveRoutes(teet *TEET, enclaveManager *shared.EnclaveManager) http.Handler {
	...
	mux.HandleFunc("/attest", func(w http.ResponseWriter, r *http.Request) {
		...
		if teet.signingKeyPair == nil {
			log.Printf("[TEE_T] No signing key pair available for attestation")
		...
		log.Printf("[TEE_T] Including ECDSA public key in attestation (DER: %d bytes)", len(publicKeyDER))
		...
		log.Printf("[TEE_T] Generated attestation document (%d bytes, base64: %d chars)",
