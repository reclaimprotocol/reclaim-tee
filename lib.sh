...
build_sample_app() {
    ...
    # For the demo, we'll use both:
    # 1. -L to specify where to find the library at compile time
    # 2. -rpath to specify where to find the library at runtime
    # Using absolute path for simplicity
    ABSOLUTE_LIB_PATH="$(cd ../$BIN_DIR && pwd)"
    export CGO_ENABLED=1
    export CGO_CFLAGS="-I."
    export CGO_LDFLAGS="-L$ABSOLUTE_LIB_PATH -lreclaim -Wl,-rpath,$ABSOLUTE_LIB_PATH"
