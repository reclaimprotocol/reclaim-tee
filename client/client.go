...
func (c *Client) handleMessages() {
	for {
		...
		case "signed_redacted_decryption_stream":
...
// calculateRedactionBytes calculates what decryption stream bytes should be to produce '*' when XORed with ciphertext
func (c *Client) calculateRedactionBytes(ciphertext []byte, startOffset, length int) []byte {
	...
	for i := 0; i < length; i++ {
		...
		if pos < len(ciphertext) {
			// redacted_stream_byte = ciphertext_byte XOR '*'
			// This ensures: ciphertext_byte XOR redacted_stream_byte = '*'
...
func (c *Client) analyzeHTTPRedactionWithBytes(httpData []byte, baseOffset int, ciphertext []byte) []shared.RedactionRange {
	...
	for _, p := range sensitivePatterns {
		...
		for {
			...
			// Calculate redaction bytes for this range
			redactionBytes := c.calculateRedactionBytes(ciphertext, absoluteIndex, rangeLength)
