...
func (c *Client) handleAttestationResponse(msg *Message) {
	...
	if attestResp.Success {
		fmt.Printf("[Client] Received successful attestation response (%d bytes)\n", len(attestResp.AttestationDoc))
		// Store response for the session (simplified without request ID matching)
		// The calling code can retrieve this based on the session
	} else {
		fmt.Printf("[Client] Received failed attestation response: %s\n", attestResp.ErrorMessage)
	}
}
