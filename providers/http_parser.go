...
type HTTPParsedResponse struct {
	...
	Chunks              []shared.RequestRedactionRange // for chunked responses
...
func NewHTTPResponseParser() *HTTPResponseParser {
	TraceStart("Parser", "NewHTTPResponseParser", "Creating streaming parser")
	...
	return &HTTPResponseParser{
		Response: &HTTPParsedResponse{
			...
			BodyStartIndex:      -1,
			Body:                []byte{},
			HeaderLowerToRanges: make(map[string]shared.RequestRedactionRange),
			Headers:             make(map[string]string),
			Chunks:              []shared.RequestRedactionRange{},
...
func (p *HTTPResponseParser) OnChunk(data []byte) error {
	TraceVerbose("Parser", "OnChunk", "Processing chunk of %d bytes", len(data))

	if p.complete {
		TraceError("Parser", "OnChunk", "Received data after response was marked complete")
	...
	TraceVerbose("Parser", "OnChunk", "Total buffered data: %d bytes", len(p.remaining))
...
func (p *HTTPResponseParser) StreamEnded() error {
	TraceStart("Parser", "StreamEnded", "Finalizing response parsing")

	p.streamEnded = true

	// Validate response completeness
	if !p.headersComplete {
		TraceError("Parser", "StreamEnded", "Stream ended before headers were complete")
	...
	if p.remainingBodyBytes > 0 {
		TraceError("Parser", "StreamEnded", "Stream ended with %d body bytes still expected", p.remainingBodyBytes)
	...
	if len(p.remaining) > 0 {
		if p.remainingBodyBytes == -1 {
			...
			TraceDebug("Parser", "StreamEnded", "Consuming final %d bytes as body for stream-until-close response", len(p.remaining))
			...
			TraceDebug("Parser", "StreamEnded", "Consuming %d remaining bytes as chunked trailer to match TypeScript", len(p.remaining))
			...
			TraceDebug("Parser", "StreamEnded", "Ignoring %d extra bytes after Content-Length body", len(p.remaining))
			p.remaining = []byte{}
		}
	}
	...
	TraceInfo("Parser", "StreamEnded", "Response parsing completed - Status: %d, Body: %d bytes, Chunks: %d",
		p.Response.StatusCode, len(p.Response.Body), len(p.Response.Chunks))
...
func (p *HTTPResponseParser) processHeaders() error {
	TraceStep("Parser", "processHeaders", 1, 2, "Processing HTTP headers")

	for {
		line, found := p.getLine()
		if !found {
			TraceVerbose("Parser", "processHeaders", "Need more data to complete header line")
...
func (p *HTTPResponseParser) parseStatusLine(line string) error {
	TraceVerbose("Parser", "parseStatusLine", "Parsing status line: '%s'", line)

	// Parse HTTP/1.1 200 OK format
	parts := strings.SplitN(line, " ", 3)
	if len(parts) < 2 {
		TraceError("Parser", "parseStatusLine", "Invalid status line format")
		return fmt.Errorf("invalid HTTP status line: %s", line)
	}

	statusCode, err := strconv.Atoi(parts[1])
	if err != nil {
		TraceError("Parser", "parseStatusLine", "Invalid status code: %s", parts[1])
	...
	TraceDebug("Parser", "parseStatusLine", "Parsed status: %d %s", p.Response.StatusCode, p.Response.StatusMessage)
	return nil
}
...
func (p *HTTPResponseParser) parseHeaderLine(line string) error {
	...
	if colonIdx == -1 {
		TraceWarn("Parser", "parseHeaderLine", "Header line missing colon separator: '%s'", line)
	...
	p.Response.HeaderLowerToRanges[key] = shared.RequestRedactionRange{
		Start:  lineStart,
		Length: len(line),
		Type:   "sensitive",
	}

	TraceVerbose("Parser", "parseHeaderLine", "Header '%s': '%s' at [%d:%d]", key, value, lineStart, lineStart+len(line))
	return nil
}

// finishHeaders completes header processing and sets up body parsing
func (p *HTTPResponseParser) finishHeaders() error {
	TraceStep("Parser", "processHeaders", 2, 2, "Finalizing headers and setting up body parsing")
	...
	if strings.Contains(strings.ToLower(transferEncoding), "chunked") {
		...
		TraceDebug("Parser", "finishHeaders", "Using chunked transfer encoding")
		...
		if length == 0 {
			p.complete = true
			p.Response.Complete = true
			TraceInfo("Parser", "finishHeaders", "Zero-length body, response complete")
		}
	} else {
		// Read until connection closes
		p.remainingBodyBytes = -1
		TraceDebug("Parser", "finishHeaders", "Using read-until-close strategy")
	}
...
func (p *HTTPResponseParser) processFixedBody() error {
	if p.remainingBodyBytes == 0 {
		...
		if !p.complete {
			p.complete = true
			p.Response.Complete = true
			TraceInfo("Parser", "processFixedBody", "Fixed-length body complete")
		}
		return nil
	}
	...
	if p.remainingBodyBytes == -1 {
		...
		TraceVerbose("Parser", "processFixedBody", "Reading %d bytes for stream-until-close", bytesToCopy)
	} else {
		...
		TraceVerbose("Parser", "processFixedBody", "Reading %d bytes, %d remaining", bytesToCopy, p.remainingBodyBytes)
	}
	...
	if p.remainingBodyBytes == 0 {
		p.complete = true
		p.Response.Complete = true
		TraceInfo("Parser", "processFixedBody", "Content-Length body complete")
	}
...
func (p *HTTPResponseParser) processChunkedBody() error {
	TraceVerbose("Parser", "processChunkedBody", "Processing chunked body with %d bytes buffered", len(p.remaining))
	...
	for {
		if p.remainingBodyBytes > 0 {
			...
			if !bytes.Equal(p.remaining[:2], []byte("\r\n")) {
				TraceError("Parser", "processChunkedBody", "Missing chunk trailing CRLF")
				return errors.New("invalid chunk: missing CRLF after data")
			}
			TraceVerbose("Parser", "processChunkedBody", "Consuming chunk trailing CRLF at position %d", p.currentByteIdx)
		...
		TraceVerbose("Parser", "processChunkedBody", "Read chunk size line: '%s' at position %d", line, p.currentByteIdx)

		if line == "" {
			TraceVerbose("Parser", "processChunkedBody", "Skipping empty line at position %d", p.currentByteIdx)
		...
		if err != nil {
			TraceError("Parser", "processChunkedBody", "Invalid chunk size: %s", sizeStr)
			return fmt.Errorf("invalid chunk size %s: %w", sizeStr, err)
		}

		TraceVerbose("Parser", "processChunkedBody", "Next chunk size: %d bytes", chunkSize)

		if chunkSize == 0 {
			// Final chunk - response is complete, but continue processing remaining lines like TypeScript
			p.complete = true
			p.Response.Complete = true
			TraceInfo("Parser", "processChunkedBody", "Final chunk (size 0) found, processing remaining lines")
			...
			for {
				...
				TraceVerbose("Parser", "processChunkedBody", "Processing post-final-chunk line: '%s'", line)
				// Empty lines and other data after final chunk are consumed but ignored
			}

			TraceInfo("Parser", "processChunkedBody", "Chunked response processing complete at position %d", p.currentByteIdx)
			break
		}

		// Record chunk range (absolute positions in complete response)
		chunkStart := p.currentByteIdx
		TraceVerbose("Parser", "processChunkedBody", "Recording chunk: start=%d, size=%d, currentByteIdx=%d", chunkStart, chunkSize, p.currentByteIdx)
		p.Response.Chunks = append(p.Response.Chunks, shared.RequestRedactionRange{
			Start:  chunkStart,
...
func StreamingHTTPResponseExample() {
	TraceInfo("Parser", "Example", "Demonstrating streaming HTTP response parsing")
	...
	if err := parser.OnChunk(chunk1); err != nil {
		TraceError("Parser", "Example", "Error processing chunk 1: %v", err)
		return
	}

	// Process second chunk
	if err := parser.OnChunk(chunk2); err != nil {
		TraceError("Parser", "Example", "Error processing chunk 2: %v", err)
		return
	}

	// Signal end of stream
	if err := parser.StreamEnded(); err != nil {
		TraceError("Parser", "Example", "Error finalizing response: %v", err)
		return
	}

	TraceInfo("Parser", "Example", "Successfully parsed streaming response: Status %d, Body: '%s'",
		parser.Response.StatusCode, string(parser.Response.Body))
}
